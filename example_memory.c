 ...
 *     If input "Write memory", that will write data to the allocated memory.
 *     If input "Read memory", that will read data from the allocated memory.
 * 
 * Author:
 * -------
 * -------
...
static void CallBack_UART_Hdlr(Enum_SerialPort port, Enum_UARTEventType msg, bool level, void* customizedPara)
{
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            rdLen = ReadSerialPort(port, m_Read_Buffer, sizeof(m_Read_Buffer));
            pData = m_Read_Buffer;
            ...
            p = Ql_strstr(pData,"Write memory");
            if (p)
            {
				if(pointer != NULL)
				{
