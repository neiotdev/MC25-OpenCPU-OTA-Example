 ...
 *   Operation:
 *            
 *     If input "ql_spi_rd_flash_id1", that will read the flash's ID.
 ...
 *     If input "ql_spi_rd_flash_id2", that will read the flash's ID by full-duplex API.
...
//spi device select£¬only for simulating SPI by software.
void spi_flash_cs(bool CS)
{
	if (!spi_usr_type)
	{
		if (CS)
			Ql_GPIO_SetLevel(PINNAME_SD_CMD,PINLEVEL_HIGH);
		else
			Ql_GPIO_SetLevel(PINNAME_SD_CMD,PINLEVEL_LOW);
	}
}
...
void proc_main_task(void)
{
    ...
    ret = Ql_SPI_Init(USR_SPI_CHANNAL,PINNAME_SD_CLK,PINNAME_SD_DATA2,PINNAME_SD_DATA1,PINNAME_SD_CMD,spi_usr_type);
    ...
    ret = Ql_SPI_Config(USR_SPI_CHANNAL,1,0,0,10); //config sclk about 10MHz;
    if(ret <0)
    {
	...
	if (!spi_usr_type)
	{
		Ql_GPIO_Init(PINNAME_SD_CMD,PINDIRECTION_OUT,PINLEVEL_HIGH,PINPULLSEL_PULLUP);   //CS high
	}
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            {        
                ...
                rdLen =ReadSerialPort(port, Read_Buffer, sizeof(Read_Buffer));
                if (!rdLen)
                {
                    APP_DEBUG("\r\nread error.\r\n");
                    break;
