...
#define APP_DEBUG(FORMAT,...) {\
    ...
    Ql_snprintf(DBG_BUFFER,DBG_BUF_LEN,FORMAT,##__VA_ARGS__); \
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            if(UART_PORT1 == port || UART_PORT2 == port)
            {
                ...
                rd_len = ReadSerialPort(port, phy_uart_buff, sizeof(phy_uart_buff));
                if (!rd_len)
                {
                    APP_DEBUG("\r\nread error.\r\n");
                    break;
            ...
            else if(VIRTUAL_PORT1 == port)
            {
                ...
                rd_len = ReadSerialPort(port, vir_uart_buff, sizeof(vir_uart_buff));
                if (!rd_len)
                {
                    APP_DEBUG("\r\nread error.\r\n");
                    break;
